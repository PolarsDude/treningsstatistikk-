...
from plotnine import ggplot,geom_col, geom_hline,aes, geom_bar,geom_line,scale_y_continuous ,labs, coord_flip, theme_bw
...
# Hent ut gjennomsnitt
gj_antall_deltakere_2024 = antall_deltakere.select('År','Snitt per periode').filter(pl.col('År')=='2024').drop('År').unique().to_series()[0]

gj_antall_deltakere_2025 = antall_deltakere.select('År','Snitt per periode').filter(pl.col('År')=='2025').drop('År').unique().to_series()[0]

(
    ggplot(antall_deltakere, aes(x="Dato", y="Antall deltakere",color = 'År'))
    + geom_line()
    +scale_y_continuous(breaks=np.arange(0, antall_deltakere["Antall deltakere"].max()+1, 1))
    +theme_bw()
    +geom_hline(yintercept=gj_antall_deltakere_2024,color = 'red',linetype = 'dashed')
    +geom_hline(yintercept=gj_antall_deltakere_2025,color = 'blue',linetype = 'dashed')
    +labs(x = "",y = "",title="Antall deltakere fordelt på dato og periode.\nBlå horisontal linje er gjennomsnittlig antall deltakere\nfor 2025, mens rød er for 2024.")
)
